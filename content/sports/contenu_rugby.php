...
$sth->execute(array(mktime(0, 0, 0, date('n'), date('j'), date('Y')) - ((date('N') - 1) * 3600 * 24), mktime(0, 0, 0, date('n'), date('j'), date('Y')) - ((date('N') - 1) * 3600 * 24) + 7 * 24 * 3600, 'rugby'));
$reponse = null;
$matchsSemaine = array();
...
require_once('scripts/utilsBD.php');
$dbh = Database::connect();
$requete = "SELECT * FROM games,resultats WHERE games.sport = 'rugby' AND games.id = resultats.id ORDER BY games.dateDebut DESC";
    ...
    <div class="col-lg-6" style="float: right">
        ...
        <div class="panel panel-accueil" style="padding-left: 0px; padding-right: 0px;">
            ...
            <div class="panel-body">
                    ...
                    ?>
                <p style="text-align: center; font-size: bigger; font-weight: bolder">Pour tous les résultats du Rugby, c'est par <a href="index.php?page=content/resultats/rugby">ici</a></p>
            </div>
        </div>
    </div>

    <p>La section rugby a été sacrée championne Ile de France des Grandes Ecoles en 2012</p>

    <p><?php
                    $dbh = Database::connect();
                    $query = "SELECT * FROM `teams` WHERE sport LIKE 'rugby'";
...
if (isset($_SESSION['estRespoSport']) && ($_SESSION['estRespoSport'] == "oui" || ($_SESSION['estCapitaine'] === "oui" && $_SESSION['section'] === "rugby")))
...
?>
    <div class="formulaire" id="formulaire1">
        <form action='index.php?page=content/sports/contenu_rugby' method='post'>
