...
$sth->execute(array(mktime(0, 0, 0, date('n'), date('j'), date('Y')) - ((date('N') - 1) * 3600 * 24), mktime(0, 0, 0, date('n'), date('j'), date('Y')) - ((date('N') - 1) * 3600 * 24) + 7 * 24 * 3600, 'badminton'));
$reponse = null;
$matchsSemaine = array();
...
require_once('scripts/utilsBD.php');
$dbh = Database::connect();
$requete = "SELECT * FROM games,resultats WHERE games.sport = 'badminton' AND games.id = resultats.id ORDER BY games.dateDebut DESC";
    ...
    <div class="col-lg-6" style="float: right">
        ...
        <div class="panel panel-accueil" style="padding-left: 0px; padding-right: 0px;">
            ...
            <div class="panel-body">
                    ...
                    ?>
                <p style="text-align: center; font-size: bigger; font-weight: bolder">Pour tous les résultats du Badminton, c'est par <a href="index.php?page=content/resultats/badminton">ici</a></p>
            </div>
        </div>
    </div>

    <p>La section badminton participe à l'organisation de la nuit des 3 raquettes chaque année à l'X.</p>

    <p><?php
                    $dbh = Database::connect();
                    $query = "SELECT * FROM `teams` WHERE sport LIKE 'badminton'";
...
if (isset($_SESSION['estRespoSport']) && ($_SESSION['estRespoSport'] == "oui" || ($_SESSION['estCapitaine'] === "oui" && $_SESSION['section'] === "badminton")))
...
?>
    <div class="formulaire" id="formulaire1">
        <form action='index.php?page=content/sports/contenu_badminton' method='post'>
